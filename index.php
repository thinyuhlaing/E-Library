...
$PAGE->set_url(new moodle_url('/local/e_library/index.php'));
...
$elibrary_category_id = 12; // Replace with the actual ID of the E-library category
...
$sql = '
    ...
    WHERE c.visible = 1
    AND (cc.id = ? OR cc.path LIKE ?)';
$params = [$elibrary_category_id, '/'.$elibrary_category_id.'/%'];
...
<div class="tabs-elibrary">
    <ul id="subCategoryFilterTabs" onchange="filterCoursesByCategoryTabs()">
...
function filterCoursesByCategoryTabs() {
    let categoryFilter = document.getElementById('subCategoryFilterTabs').value;
    // Update the URL to reflect the selected sub-category
    let newUrl = window.location.protocol + "//" + window.location.host + window.location.pathname + '?subcategory=' + categoryFilter;
    window.history.pushState({path: newUrl}, '', newUrl);
